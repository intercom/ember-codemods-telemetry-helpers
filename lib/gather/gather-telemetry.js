...
module.exports = async function gatherTelemetry(url, puppeteerArgs = {}) {
  ...
  const telemetry = await page.evaluate(() => {
    ...
    for (let modulePath of modules) {
      if (SKIPPED_MODULES.includes(modulePath) || modulePath.includes('template')) {
        continue;
      }
      ...
      try {
        ...
        if (module && module.default && (module.default.proto || module.default.create)) {
          ...
          if (module.default.isHelperFactory) {
            telemetry[modulePath].type = 'Helper';
          }
        }
      } catch (error) {
